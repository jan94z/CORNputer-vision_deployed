...
class realsense():
    ...
    """
    A class representing a RealSense camera with advanced mode capabilities.
    Args:
        width (int): The width of the color stream. Default is 1280.
        height (int): The height of the color stream. Default is 720.
        fps (int): The frames per second of the color stream. Default is 30.
        config (str): The path to a JSON configuration file. Default is None.
    Attributes:
        w (int): The width of the color stream.
        h (int): The height of the color stream.
        fps (int): The frames per second of the color stream.
        advnc_mode (rs.rs400_advanced_mode): The advanced mode object.
    Methods:
        _find_device_that_supports_advanced_mode(): Finds a RealSense device that supports advanced mode.
        _enable_advanced_mode(): Enables advanced mode for the RealSense device.
        _load_json(path: str): Loads a JSON configuration file.
        save_params2config(path: str): Saves the current advanced mode parameters to a JSON configuration file.
        print_current_params(): Prints the current advanced mode parameters.
        start(): Starts the RealSense pipeline.
        get_frame(): Retrieves a color frame from the RealSense camera.
        stop(): Stops the RealSense pipeline.
    """


    def __init__(self, width = 1280, height = 720, fps = 30, config=None):
        """
        Initializes the RS class.
        Parameters:
        - width (int): The width of the image frame (default: 1280).
        - height (int): The height of the image frame (default: 720).
        - fps (int): The frames per second of the camera (default: 30).
        - config (str): The path to the JSON configuration file (default: None).
        """
    ...
    def _load_json(self, path):
        """
        Loads a JSON file from the given path and applies the configuration to the advanced mode of the RealSense camera.
        Args:
            path (str): The path to the JSON file.
        Returns:
            None
    ...
    def start(self):
        """
        Starts the RealSense pipeline with the specified configuration.
        Returns:
            None
        """
    ...
    def get_frame(self):
        """
        Retrieves a color image frame from the RealSense camera.
        Returns:
            numpy.ndarray: The color image frame as a NumPy array.
        """
