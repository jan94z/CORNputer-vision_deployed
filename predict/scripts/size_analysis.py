...
class SizeEstimator:
    ...
    def predict(self, batch_size=8, save_img=None):
        ...
        for fp in tqdm(range(0, len(self.fps), batch_size), "Size Estimation"):
    ...
    def _mbr_analysis(self, binary_mask):
        ...
        edge_lengths = [np.linalg.norm(box[i] - box[(i + 1) % 4]) for i in range(4)] # Calculate lengths of edges
        length_idx = np.argmax(edge_lengths)  # Find the longest edge
        width_idx = (length_idx + 1) % 4  # Width is perpendicular to the length

        length = edge_lengths[length_idx] # Major axis
        width = edge_lengths[width_idx] # Minor axis
